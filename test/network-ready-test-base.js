...
const EXISTING_ADDRESS_BOOK_FILE_ID = null;
const EXISTING_ADDRESS_BOOK_JSON = null;
...
class NetworkReadyTestBase {
    ...
    async setup() {
        ...
        const abJson = EXISTING_ADDRESS_BOOK_JSON;
        const abFileId = EXISTING_ADDRESS_BOOK_FILE_ID;
        if (!(abJson) || !(abFileId)) {
            await this.setupIdentityNetwork();
        } else {
            const addressBook = AddressBook.fromJson(abJson, FileId.fromString(abFileId));
            this.didNetwork = HcsIdentityNetwork.fromAddressBook(this.network, addressBook);
        }
    }
    ...
    async setupIdentityNetwork() {
        ...
        console.info("Sleeping 10s to allow propagation of new topics to mirror node");
    ...
    async sendDidTransaction(did, didDocumentJson, operation, onError) {
        ...
        await this.didNetwork.createDidTransaction(operation)
    ...
    async resolveDid(didString, onError) {
        ...
        // Now resolve the DID.
        this.didNetwork.getDidResolver()
            .addDid(didString)
    ...
    async sendVcTransaction(operation, credentialHash, signingKey, onError) {
        ...
        // Build and execute transaction
        await this.didNetwork.createVcTransaction(operation, credentialHash, signingKey.publicKey)
    ...
    async resolveVcStatus(credentialHash, provider, onError) {
        ...
        // Now resolve the DID.
        this.didNetwork.getVcStatusResolver(provider)
