...
describe("DidDocumentBase", function () {
  it("Test Serialization", async function () {
    ...
    assert.equal(
      root[DidDocumentJsonProperties.CONTEXT],
  ...
  it("Test Deserialization", async function () {
    ...
    assert.equal(
      ...
      doc.getDidRootKey().getPublicKeyBase58()
    );
    assert.equal(
      didRootKey.getController(),
      doc.getDidRootKey().getController()
    );
