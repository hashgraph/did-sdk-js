...
describe('HcsDidRootKey', function() {
    it('Test Generate', async function() {
        const addressBook = '0.0.1';

        const privateKey = HcsDid.generateDidRootKey();

        const did = new HcsDid(network, privateKey.publicKey, FileId.fromString(addressBook));

        assert.throw(() => {HcsDidRootKey.fromHcsIdentity(null, null);});
        assert.throw(() => {HcsDidRootKey.fromHcsIdentity(did, null);});
        assert.throw(() => {HcsDidRootKey.fromHcsIdentity(null, privateKey.publicKey);});

        const differentPublicKey = HcsDid.generateDidRootKey().publicKey;
        assert.throw(() => {HcsDidRootKey.fromHcsIdentity(did, differentPublicKey);});
