...
const { AccountId, PrivateKey, Client, Hbar, TopicInfoQuery, TopicId } = require("@hashgraph/sdk");

const { HcsDid, HcsIdentityNetworkBuilder, HcsIdentityNetwork } = require("../dist");
...
describe("HcsIdentityNetwork", function () {
    ...
    it("Test Create Identity Network", async function () {
        ...
        const didTopicInfo = await new TopicInfoQuery().setTopicId(didNetwork.getDidTopicId()).execute(client);

        assert.exists(didTopicInfo);
        assert.equal(didTopicInfo.topicMemo, didTopicMemo);

        const vcTopicInfo = await new TopicInfoQuery().setTopicId(didNetwork.getVcTopicId()).execute(client);
    ...
    it("Test Init Network From Did topic", async function () {
        this.timeout(60000);
        const did = new HcsDid(network, HcsDid.generateDidRootKey().publicKey, DID_TOPIC_ID);

        const didNetwork = await HcsIdentityNetwork.fromHcsDidTopic(network, did.getDidTopicId());
    ...
    it("Test Generate Did For Network", async function () {
        ...
        const didNetwork = await HcsIdentityNetwork.fromHcsDidTopic(network, DID_TOPIC_ID);
